...
void HAL_FLASH_Erase(HAL_FLASH_x flash, uint32_t flashaddr, uint32_t len)
{
    (void)flash;
    uint32_t len1 = len;
    HAL_REG_WRITE_BIT(FLASH_CFG, BIT31 | BIT15 | BIT11, BIT31);
    for (; len1 > 0; len1 -= 512, flashaddr += 512)
    {
        HAL_REG_WRITE(FLASH_ADDR, flashaddr);
        HAL_REG_WRITE(FLASH_ERASE, 0x7654dcba);
    }
    HAL_REG_WRITE_BIT(FLASH_CFG, BIT31 | BIT15 | BIT11, 0);
}
...
void HAL_FLASH_Program(HAL_FLASH_x flash, uint32_t flashaddr, uint8_t *data, uint32_t len)
{
    (void)flash;
    uint32_t len1 = len;
    HAL_REG_WRITE_BIT(FLASH_CFG, BIT27 | BIT23 | BIT11, BIT27 | BIT23);
    flashaddr &= ~0x3;
    for (; len1 > 0; len1 -= 4, flashaddr += 4)
    {
        uint32_t val;
        val = data[0] | (data[1] << 8) | (data[2] << 16) | (data[3] << 24);
        HAL_REG_WRITE(FLASH_ADDR, flashaddr);
        HAL_REG_WRITE(FLASH_WDATA, val);
    }
    HAL_REG_WRITE_BIT(FLASH_CFG, BIT27 | BIT23 | BIT11, 0);
}
