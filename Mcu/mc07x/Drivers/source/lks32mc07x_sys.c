...
/**
 * @brief 设置芯片工作电压（同时打开电源掉电检测中断）
 * @param SYS_MCU_POWER_x 芯片供电电压 SYS_MCU_POWER_3v3 或 SYS_MCU_POWER_5v0
 */
void SYS_VolSelModule(uint32_t SYS_MCU_POWER_x)
{

    if (SYS_MCU_POWER_x == MCU_POWER_5v0) // 5.0 Voltage
    {
        SYS_WR_PROTECT = 0x7a83;             /* 解除系统寄存器写保护 */
        SYS_AFE_REG6 &= ~(BIT8 | BIT9);      // 恢复初始配置
        SoftDelay(100);                      /* 延时100us*/
        SYS_AFE_REG6 |= SYS_POWER_3V75 << 8; // 配置为3.75V监控，可根据实际配置其它档位：3.25V/3.5V/3.75V/4.0V
        ...
        SYS_AFE_REG6 &= ~(BIT8 | BIT9);      // 恢复初始配置
        SoftDelay(100);                      /* 延时100us*/
        SYS_AFE_REG6 |= SYS_POWER_3V25 << 8; // 配置为3.25V，防止电源和欠压比较的抖动
        SoftDelay(100);                      /* 延时100us*/
        NVIC_ClearPendingIRQ(PWRDN_IRQn);
        SYS_WR_PROTECT = 0x0; /*关闭系统寄存器写操作*/
    }
    else if ((SYS_AFE_INFO & 0xf) >= 4) // 芯片版本大于等于4时，可以用电源掉电检测电路监测3.3V的电压做欠压保护，小于该版本无法检测3.3V电压
    {
        SYS_WR_PROTECT = 0x7a83;            // 解除系统寄存器写保护
        SYS_AFE_REG6 &= ~(BIT8 | BIT9);     // 恢复初始配置
        SYS_AFE_REG6 |= SYS_POWER_2V5 << 8; // 配置为2.5V欠压保护
        SoftDelay(100);                     // 延时100us
        while (SYS_AFE_DBG & BIT15)         // 等待外部2.5VOK
        {
        };
        SoftDelay(100); // 延时100us
        NVIC_ClearPendingIRQ(PWRDN_IRQn);
...
void SYS_VolSelModuleIRQ(void)
{
    ...
    NVIC_ClearPendingIRQ(PWRDN_IRQn);
    // turn off pwm
    SYS_WR_PROTECT = 0x7A83; /* 时钟选HRC */
    SYS_CLK_CFG    = 0x0000;
    for (cnt = 0; cnt < 4000; cnt++)
    {
        __NOP();
    }
    SYS_AFE_REG6 &= ~(BIT8 | BIT9); // this clause reset to 4.0v
    SYS_WR_PROTECT = 0x0000;
    MCPWM0_PRT     = 0xDEAD; /* 关电机 */
    MCPWM0_FAIL012 &= (~0x0040);
    MCPWM0_FAIL345 &= (~0x0040);
    MCPWM0_PRT = 0x0;
    NVIC_SystemReset(); // 软复位
}
/**
 * @brief 使能掉电检测中断，需要先配置芯片工作电压(函数：SYS_VolSelModule)
 */
void SYS_VolSelModuleEnableIRQ(uint32_t SYS_MCU_POWER_x)
{
    if (SYS_MCU_POWER_x == MCU_POWER_5v0)
    {
